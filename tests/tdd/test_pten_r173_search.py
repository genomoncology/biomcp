...
async def test_pten_r173_search_limitations():
    ...
    request_restrictive = PubmedRequest(
        genes=['PTEN'],
        keywords=['R173', 'Arg173']
    )
    result_restrictive = await search_articles(request_restrictive, output_json=True)
    data_restrictive = json.loads(result_restrictive)

    # Test 2: Less restrictive approach
    request_less_restrictive = PubmedRequest(
        genes=['PTEN'],
        keywords=['R173']
    )
    result_less_restrictive = await search_articles(request_less_restrictive, output_json=True)
    data_less_restrictive = json.loads(result_less_restrictive)

    # Test 3: Alternative variant notations
    request_notation = PubmedRequest(
        genes=['PTEN'],
        keywords=['p.R173C']
    )
    ...
    print(f"1. PTEN + R173 + Arg173 (AND logic): {len(data_restrictive)} articles")
...
async def test_specific_pten_papers_not_found():
    ...
    for article in data:
        ...
        if 'R173' in title or 'R173' in abstract or 'Arg173' in title or 'Arg173' in abstract:
    ...
    assert len(r173_papers) < len(expected_papers), "Not all expected R173 papers are found"
