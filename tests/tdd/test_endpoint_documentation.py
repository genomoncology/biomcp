...
class TestEndpointDocumentation:
    ...
    def test_third_party_endpoints_file_exists(self):
        ...
        endpoints_file = Path(__file__).parent.parent.parent / "THIRD_PARTY_ENDPOINTS.md"
        assert endpoints_file.exists(), "THIRD_PARTY_ENDPOINTS.md must exist"

    def test_endpoints_documentation_is_current(self):
        """Test that the endpoints documentation matches the registry."""
        # Run the generation script
        script_path = Path(__file__).parent.parent.parent / "scripts" / "generate_endpoints_doc.py"
        ...
        endpoints_file = Path(__file__).parent.parent.parent / "THIRD_PARTY_ENDPOINTS.md"
        current_content = endpoints_file.read_text()

        # Generate fresh content
        from biomcp.utils.endpoint_registry import get_registry
        registry = get_registry()
        expected_content = registry.generate_markdown_report()

        # Compare (this will fail if someone modified the file without updating the registry)
        assert current_content == expected_content, (
            "THIRD_PARTY_ENDPOINTS.md is out of sync with the registry. "
            "Run 'python scripts/generate_endpoints_doc.py' to update it."
        )
    ...
    def test_all_endpoints_documented(self):
        ...
        for key, info in endpoints.items():
            ...
            assert info.url in content, f"URL {info.url} not found in documentation"
