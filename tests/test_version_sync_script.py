...
def test_version_sync_script_passes_for_development_candidate_split(tmp_path: Path) -> None:
    ...
    assert rust_version == "0.9.0-dev.5"
    assert python_version == "0.9.0.dev5"
    ...
    assert result.stdout.strip() == (
        "Versions in sync: 0.9.0-dev.5 "
        "(Python 0.9.0.dev5; development candidate)"
    )
...
def test_version_sync_script_reports_pyproject_mismatch(tmp_path: Path) -> None:
    ...
    assert result.returncode == 1
    assert (
        "Version mismatch: Cargo.toml mapping=0.9.0.dev5, "
...
def test_manifest_and_citation_versions_match_repo_metadata() -> None:
    ...
    assert cargo["package"]["version"] == "0.9.0-dev.5"
    assert pyproject["project"]["version"] == "0.9.0.dev5"
    assert _read_manifest_version(REPO_ROOT / "manifest.json") == "0.8.25"
    assert _read_server_versions(REPO_ROOT / "server.json") == ("0.8.25", "0.8.25")
    assert _read_citation_version(REPO_ROOT / "CITATION.cff") == "0.8.25"
...
def test_uv_lock_matches_release_version_and_excludes_mustmatch_package() -> None:
    ...
    assert root_match.group(2) == "0.9.0.dev5"
