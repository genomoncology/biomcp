...
MAX_PACKAGE_FILES = 1_348  # 1,300 base + ticket 1145 five modules + ticket 1142 orcid tests submodule + ticket 1199 cache module and tests submodule + ticket 1200 opencitations source module + ticket 1202 cellosaurus source, cell-line entity, render, CLI, docs, and spec files, minus the retired staged release workflow test, plus ticket 1214 cell-line ChEMBL section module and its tests submodule, plus ticket 1213 gene cell-lines module, its tests submodule, and its markdown template, plus ticket 1205 PharmacoDB source module and its three tests submodules, the shared PharmacoDB entity module and its tests, the cell-line drug_response and drug cell_lines section modules with their tests, the PharmacoDB row renderer and its template, the PharmacoDB source page, and the CLI selectors module split out under the line cap, plus ticket 1226 docs-live gate helper and its contract test, plus ticket 1221 CA-bundle helper and its TLS contract test, plus ticket 1233 changelog coverage helper and its contract test
