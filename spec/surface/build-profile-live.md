...
```bash
cargo run --locked --release --no-default-features --bin biomcp -- --json health --apis-only | jq '[.rows[] | select(.api == "AlphaGenome")][0] | (.status == "unavailable (not built)" and .latency == "-" and (has("key_configured") | not))' | mustmatch 'true'
```
