...
A literal BRAF gene trial pivot still returns the first trial result through the
CTGov search path, but the request sent for that one row should not include the
CTGov `countTotal` option.

```bash
../../spec/fixtures/ctgov-request-log run ../../tools/biomcp-ci --json gene trials BRAF --limit 1 \
...
```bash
../../spec/fixtures/ctgov-request-log run ../../tools/biomcp-ci --json disease trials melanoma --limit 1 \
  | jq -r '.count, (.results[0].nct_id | startswith("NCT"))' \
  | mustmatch like '1
true'
```
