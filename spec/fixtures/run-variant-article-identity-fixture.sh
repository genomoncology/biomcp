...
jq -n --argjson all "$all" --argjson confirmed "$confirmed" --argjson reserved "$reserved" --argjson reordered "$reordered" '
  ...
  def has($id; $pmid; $status): any(item($id).results[]; .pmid == $pmid and .identity.status == $status);
  {
    ...
    debug_plan_records_discovery_and_verification_allocation: (reserved_item | .debug_plan.work_allocation as $allocation | ($allocation | type) == "object" and ($allocation.discovery.consumed | type) == "number" and $allocation.discovery.consumed > 0 and $allocation.identity_verification.reserved >= 1 and $allocation.identity_verification.consumed >= 1),
