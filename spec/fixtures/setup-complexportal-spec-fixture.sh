...
recover_fixture_orphans "$cache_dir" "complexportal" "spec-complexportal."

fixture_root="$(mktemp -d "$cache_dir/spec-complexportal.XXXXXX")"
owner_arg="$(bash "$ownership_helper" new-owner "complexportal" "$fixture_root")"
...
start_fixture_supervisor "complexportal" "$cache_dir" "$fixture_root" "spec-complexportal." "$server_pid_file" \
  python3 - "$ready_file" "$request_log" "$workspace_root/testdata/sources/uniprot" "$owner_arg" <<'PY' >"$server_log" 2>&1 &
...
bash "$ownership_helper" write "$workspace_root" "complexportal" "$fixture_root" "$server_pid" "BIOMCP_COMPLEXPORTAL_FIXTURE" "$owner_arg" >/dev/null
trap - EXIT
printf '%s\n' "$fixture_root"
