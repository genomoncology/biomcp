...
PY
mcp="$(<"$tmp/mcp.json")"
jq -n --argjson summary "$summary" --argjson detail "$detail" --argjson pten "$pten" --argjson miss "$miss" --argjson batch "$batch" --argjson mcp "$mcp" --argjson ambiguous "$ambiguous" '{
  ...
  comment_strength_is_not_applied_strength: (($summary.items[0].assertions[0].criteria[] | select(.source_token == "PS4").explicit_strength) == null),
  ...
  selected_detail_keeps_version_and_citation_locator: ($detail.items[0].assertions[0].detail.criteria[0].locator | startswith("/evidenceLine/")),
