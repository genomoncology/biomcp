...
jq -n --arg markdown "$markdown" --arg detail_markdown "$detail_markdown" --arg batch_markdown "$batch_markdown" --arg resolved_gene "$resolved_gene" --arg zero_gene "$zero_gene" --rawfile missing_gene "$tmp/missing_gene.md" --rawfile unavailable_gene "$tmp/unavailable_gene.md" --rawfile unavailable_count "$tmp/unavailable_count.md" --argjson summary "$summary" --argjson detail "$detail" --argjson pten "$pten" --argjson miss "$miss" --argjson multiple "$multiple" --argjson batch "$batch" --argjson mcp "$mcp" --argjson gene "$gene" --argjson gene_second "$gene_second" --argjson gene_mcp "$gene_mcp" --argjson ambiguous "$ambiguous" --arg requests "$requests" '{
  ...
  markdown_labels_germline_scope_and_routes_somatic_questions_to_civic: ($markdown | contains("germline") and contains("get gene <symbol> civic") and contains("get variant <id> civic")),
  ...
  unavailable_gene_count_keeps_the_successful_empty_result: ($unavailable_count == "# ClinGen ERepo expert assertions\n\nClinGen ERepo contains germline variant interpretations. For somatic tumor questions, use CIViC: get gene <symbol> civic or get variant <id> civic.\n\n## CA000076\n\nNo expert assertions were returned.\n\nGene: FAILGEN\n\n"),
  empty_caid_with_no_car_gene_keeps_the_empty_result: (($missing_gene == "# ClinGen ERepo expert assertions\n\nClinGen ERepo contains germline variant interpretations. For somatic tumor questions, use CIViC: get gene <symbol> civic or get variant <id> civic.\n\n## CA000073\n\nNo expert assertions were returned.\n\n") and ($requests | contains("/allele/CA000073?fields="))),
  unavailable_car_gene_lookup_keeps_the_empty_result: (($unavailable_gene == "# ClinGen ERepo expert assertions\n\nClinGen ERepo contains germline variant interpretations. For somatic tumor questions, use CIViC: get gene <symbol> civic or get variant <id> civic.\n\n## CA000074\n\nNo expert assertions were returned.\n\n") and ($requests | contains("/allele/CA000074?fields="))),
