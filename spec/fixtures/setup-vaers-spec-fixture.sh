...
recover_fixture_orphans "$cache_dir" "vaers" "spec-vaers."
...
owner_arg="$(bash "$ownership_helper" new-owner "vaers" "$fixture_root")"
...
start_fixture_supervisor "vaers" "$cache_dir" "$fixture_root" "spec-vaers." "$server_pid_file" \
  python3 - "$ready_file" "$script_dir/vaers" "$workspace_root/testdata/sources" "$owner_arg" <<'PY' >"$server_log" 2>&1 &
...
bash "$ownership_helper" write "$workspace_root" "vaers" "$fixture_root" "$server_pid" "BIOMCP_VAERS_FIXTURE" "$owner_arg" >/dev/null
printf '%s\n' "$fixture_root"
