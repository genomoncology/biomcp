...
def _matches(data: bytes) -> bool:
    text = _text(data)
    return text is not None and any(value in text.casefold() for value in FORBIDDEN)


def _allowlist(path: Path = INVENTORY) -> dict[str, str]:
    encoded = json.loads(path.read_text(encoding="utf-8"))
    return {base64.b64decode(name).decode("utf-8"): digest for name, digest in encoded.items()}
...
def scan_archive(path: Path) -> list[str]:
    ...
    with tarfile.open(path, "r:gz") as archive:
        for member in archive.getmembers():
            ...
            if source is not None and _matches(source.read()):
...
def tracked(root: Path) -> list[str]:
    output = subprocess.run(
        ["git", "ls-files", "-z", "--cached", "--others", "--exclude-standard"], cwd=root, check=True, capture_output=True
    ).stdout
    return [value.decode("utf-8") for value in output.split(b"\0") if value and (root / value.decode("utf-8")).is_file()]
...
def main() -> int:
    ...
    violations = scan_archive(args.archive) if args.archive else scan_files(args.root, tracked(args.root), args.inventory)
    if violations:
