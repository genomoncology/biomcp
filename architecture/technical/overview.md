...
- **Edition:** Rust 2024
- **Development candidate:** Rust `0.9.0-dev.5`; Python `0.9.0.dev5`. `scripts/check-version-sync.sh` validates that exact mapping and its lock files while public metadata remains at `0.8.25`.
...
v0.8.25 is the latest published release. Package versions are committed metadata, not values stamped from tags. The current private development candidate uses Cargo `0.9.0-dev.5` and Python `0.9.0.dev5`; `scripts/check-version-sync.sh` validates that mapping and its lock roots while keeping `manifest.json`, both `server.json` version fields, `CITATION.cff`, and any concrete Homebrew formula version on the latest reachable stable tag.
