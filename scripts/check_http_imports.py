...
HTTP_LIBRARIES = {"httpx", "aiohttp", "requests", "urllib3"}  # Note: urllib is allowed for URL parsing
            ...
            if isinstance(node, ast.Import):
                for alias in node.names:
                    ...
                    if module_name in HTTP_LIBRARIES:
                        ...
                        pattern_key = (file_path.name, module_name)
...
def find_python_files(root_dir: Path) -> list[Path]:
    ...
    for path in root_dir.rglob("*.py"):
        ...
        if any(part.startswith(".") or part in ["__pycache__", "venv", "env", ".tox"] 
...
def main():
    ...
    if all_violations:
        ...
        print("If you need to add an exception, update ALLOWED_FILES or ALLOWED_PATTERNS in this script.")
        return 1
    else:
