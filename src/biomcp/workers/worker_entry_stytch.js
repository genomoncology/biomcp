...
async function validateToken(token, env) {
    ...
    try {
        ...
        try {
            ...
            log(`Self-issued JWT validation failed, trying Stytch validation: ${error.message}`);
...
async function processAuthCallback(c, token, state, oauthRequest) {
    ...
    try {
        ...
        log(`Using Stytch endpoint: ${endpoint} with payload: ${JSON.stringify(payload)}`);

        const authenticateResp = await fetch(
            getStytchUrl(c.env, endpoint),
            {
                method: "POST",
                headers: {
        ...
        log(`Auth data received: ${JSON.stringify({
...
async function proxyPost(req, remoteServerUrl, path, sid) {
    ...
    const target = `${remoteServerUrl}${path}?session_id=${encodeURIComponent(sid)}`;
    ...
    try {
        ...
        log(`Proxying ${jsonBody.method || "request"} with timeout ${timeout}ms to: ${target}`);
        ...
        const responseText = await resp.text();
        log(`Response from MCP server (first 200 chars): ${responseText.substring(0, 200)}...`);
        ...
        if (jsonBody.method === "resources/list") {
            ...
            try {
                ...
                if (!responseObj.result || !responseObj.result.resources || responseObj.result.resources.length === 0) {
...
function serveSSE(clientReq, remoteServerUrl) {
    ...
    const stream = new ReadableStream({
        async start(ctrl) {
            ...
            if (clientReq.signal && typeof clientReq.signal.addEventListener === 'function') {
            ...
            try {
                ...
                while (true) {
                    ...
                    if (value) {
                        ...
                        log(`SSE data received (first 100 chars): ${text.substring(0, 100)}`);
...
app
    ...
    // Status endpoints
    .get('/status', c => {
        const REMOTE_MCP_SERVER_URL = c.env.REMOTE_MCP_SERVER_URL || "http://localhost:8000";
    ...
    .get('/authorize', async c => {
        try {
            ...
            log(`Search params: ${JSON.stringify(Object.fromEntries(url.searchParams))}`);
            ...
            const stytchLoginUrl = `${c.env.STYTCH_OAUTH_URL || 'https://test.stytch.com/v1/public/oauth/google/start'}?public_token=${
    ...
    .get('/callback', async c => {
        try {
            ...
            const token = url.searchParams.get('stytch_token_type') === 'oauth'
                ? url.searchParams.get('token')
                : url.searchParams.get('token') || url.searchParams.get('stytch_token');
            ...
            try {
                ...
                const timestamps = await c.env.OAUTH_KV.list({prefix: 'state_timestamp:'});
            ...
            if (state) {
                try {
                    const oauthRequestJson = await c.env.OAUTH_KV.get(`oauth_request:${state}`);
                    if (oauthRequestJson) {
            ...
            if (!oauthRequest) {
                log("No OAuth request found for most recent state, checking other requests");

                try {
                    // List all OAuth requests and use the most recent one
                    const requests = await c.env.OAUTH_KV.list({prefix: 'oauth_request:'});
                    if (requests.keys.length > 0) {
                        const oauthRequestJson = await c.env.OAUTH_KV.get(requests.keys[0].name);
                        if (oauthRequestJson) {
                            oauthRequest = JSON.parse(oauthRequestJson);
            ...
            return new Response(`Server error during authentication: ${error.message}`, {
                status: 500,
                headers: CORS
            });
        }
    ...
    .post('/token', async c => {
        try {
            ...
            const encoder = new TextEncoder();
            const secret = encoder.encode(c.env.JWT_SECRET || 'default-jwt-secret-key');
