...
async fn initial_fresh_and_conditional_304_lifecycle_uses_one_get() {
    ...
    async fn handler(
        ...
        if method == Method::GET && matches!(count, 1 | 4) {
    ...
    assert!(!client.sync().await.unwrap());
    {
        ...
        assert_eq!(request_log[0].0, Method::GET);
        assert_eq!(request_log[1].0, Method::GET);
        assert_eq!(request_log[1].1["if-none-match"], "\"fixture-v1\"");
        assert_eq!(
            request_log[1].1["if-modified-since"],
