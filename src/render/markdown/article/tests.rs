...
#[test]
fn article_graph_markdown_is_byte_exact_for_every_page_shape_in_both_directions() {
    use crate::entities::article::{
        ArticleGraphMeta, ArticleGraphPagination, ArticleGraphResult, GraphCoverageStatus,
    };

    const ROW: &str =
        "| PMID 24200969 | Related paper | Background | yes | Important supporting context |\n";
    const EMPTY_ROW: &str = "| - | - | - | - | No related papers returned |\n";
    let article = crate::entities::article::ArticleRelatedPaper {
        paper_id: Some("paper-1".into()),
    ...
    let edge = crate::entities::article::ArticleGraphEdge {
        paper: crate::entities::article::ArticleRelatedPaper {
            paper_id: Some("paper-2".into()),
            pmid: Some("24200969".into()),
            doi: None,
            arxiv_id: None,
            title: "Related paper".into(),
    ...
    let cases = [
        (
            ...
            false,
            "Page offset: 0; page size: 1; returned: 1; coverage: continuable.\nSemantic Scholar does not provide an exact total.\n",
        ),
        (
            1,
            1,
            ...
            GraphCoverageStatus::Continuable,
            false,
            "Page offset: 1; page size: 1; returned: 1; coverage: continuable.\nSemantic Scholar does not provide an exact total.\n",
        ),
        (
            58,
            1,
            ...
            false,
            "Page offset: 58; page size: 1; returned: 1; coverage: exhausted.\nSemantic Scholar does not provide an exact total.\n",
        ),
        (
            1000,
            0,
        ...
        (
            ...
            GraphCoverageStatus::Exhausted,
            true,
            "Page offset: 1001; page size: 1; returned: 0; coverage: exhausted.\nSemantic Scholar does not provide an exact total.\n",
        ),
    ];

    for (kind, direction) in [("Citations", "citations"), ("References", "references")] {
        for (offset, returned, next, status, empty, footer) in cases {
            ...
            let expected = format!(
                ...
                if empty { EMPTY_ROW } else { ROW },
                command.map_or_else(String::new, |value| format!("Next: `{value}`\n")),
            );
            assert_eq!(article_graph_markdown(kind, &result).unwrap(), expected);
        }
    }
}

#[test]
fn article_graph_markdown_uses_a_safe_code_span_for_the_shared_command() {
    let mut result = crate::entities::article::ArticleGraphResult {
        article: crate::entities::article::ArticleRelatedPaper {
            paper_id: Some("paper-1".into()),
            pmid: Some("22663011".into()),
            doi: None,
    ...
    let markdown = article_graph_markdown("Citations", &result).unwrap();
    assert!(
    ...
    assert!(
        !article_graph_markdown("Citations", &result)
            .unwrap()
            .contains("Next:")
    );
}
