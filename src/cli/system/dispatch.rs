...
pub(crate) async fn handle_batch(args: BatchArgs, json: bool) -> anyhow::Result<CommandOutcome> {
    let prepared = preflight_batch(&args)?;
    let entity = prepared.entity;
    let parsed_ids = prepared.ids;
    let batch_sections = prepared.sections;
    ...
    match entity.as_str() {
        ...
        "trial" => {
            let trial_source = prepared.trial_source.expect("trial source preflight");
            let futs = parsed_ids
                .iter()
