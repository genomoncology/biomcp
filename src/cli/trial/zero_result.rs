...
pub(super) fn verification_emptied_trial_hint(upstream_total: usize) -> String {
    format!(
        "ClinicalTrials.gov matched {upstream_total} trial(s) on this eligibility text, but registry eligibility verification removed all of them (the term appears only in exclusion criteria or outside the inclusion section). Try a shorter phrase or `--mutation` for broader field coverage."
    )
}
