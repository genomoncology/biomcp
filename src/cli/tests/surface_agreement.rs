...
fn drug_command_discovery_has_one_renderer_owner_per_surface() {
    let renderer_sources = [
        ("cli/drug/mod.rs", include_str!("../drug/mod.rs")),
        ("cli/drug/dispatch.rs", include_str!("../drug/dispatch.rs")),
    ...
    let drug_render = include_str!("../drug/render.rs");
    let drug_markdown = include_str!("../../render/markdown/drug.rs");
    assert_eq!(drug_render.matches("drug_command_discovery").count(), 2);
    assert_eq!(drug_markdown.matches("drug_command_discovery").count(), 1);
