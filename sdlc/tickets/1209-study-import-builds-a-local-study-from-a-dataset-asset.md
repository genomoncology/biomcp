...
- Ticket 1208 records the series-level `upstream_last_updated`, `status`, and `matrix_urls` in a `series` block in each dataset manifest, per asset `sha256` and a `previous` list, and a series-level `imports` list. Its writes take the series `manifest.lock`. It stores `gene_info:<organism>` and `gene_history` once under `<dataset root>/ncbi-gene/` with their own manifest.
...
- Every study command failure prints `Source unavailable: cBioPortal DataHub is not available.` in Markdown and JSON (`src/error.rs:482`, `:573`). The internal reason, such as `Missing study metadata file` (`src/sources/cbioportal_study.rs:1486`), is dropped. Ticket 1218 changes that rendering for every study command, and this ticket depends on it.
...
- NCBI counts: the gene ID column becomes `Entrez_Gene_Id`. The symbol comes from NCBI Gene `gene_info` for the organism, downloaded through 1208 (`--annotation gene_info:<organism>`, required for this producer). Ticket 1208 stores that file and `gene_history` once under `<dataset root>/ncbi-gene/` with their own manifest, so `--annotation gene_info:<organism>` and `--gene-history gene_history` resolve there and not in the series folder. A gene ID with no symbol is dropped. When `--gene-history gene_history` is given, dropped rows are split into retired IDs and replaced IDs. Otherwise they are counted as one number.
- Arrays: the matrix table (downloaded through 1208 as `matrix:<GPL>`) is read past the header by a streaming table reader added to `src/sources/geo_matrix.rs`, beside the `read_header` that ticket 1211 owns. The reader reuses that header parser for the table header line, so the sample column order comes from one place. It streams rows between `!series_matrix_table_begin` and `!series_matrix_table_end` without holding the file in memory, which matters because the largest measured table is 5,456,443 rows. It strips the surrounding quotes from each cell the way the header parser does, treats a blank cell as missing rather than as zero, and keeps a value at or below zero as published. A row whose column count differs from the header is a provider error naming the file and the row number. The GPL annotation file maps probe to symbol and gene ID. The user downloads it through 1208 as `annot:<GPL>` and passes it with `--annotation`, the same flag NCBI counts use. A platform with no annotation file fails with a message naming the platform. Multi-gene probes are split on `///`. Probes with no gene, or with several genes, are dropped and counted. `--probe-rule max-mean` is the only value today and the default. It keeps the probe with the highest mean, the `MaxMean` rule of WGCNA `collapseRows`. The mean is computed once over the samples kept by the map and never uses group labels. Blank cells are skipped in the mean. A tie goes to the lowest probe ID in byte order. `meta_study.txt` records the rule. `import.json` records the rule and the annotation file date from `!Annotation_date`.
...
- `import.json`: the full provenance, including the sample map SHA-256, the SHA-256 of every asset it read, `upstream_last_updated` copied from the `series` block of the 1208 manifest, the import time, the three unit fields, the annotation file date, dropped-row counts (retired, replaced, and other), symbol collisions, and unmatched matrix columns. `upstream_last_updated` is the series date, which ticket 1208 records for every asset, including NCBI count files and supplementary files, because GEO publishes one date per series. It is `null` only when the series header carried no update date.
- The SHA-256 import records for the value asset is the entry's top-level `sha256`, which is the file on disk at import time, never a `previous` row. A manifest entry with a `previous` list therefore has one unambiguous answer, and `dataset list` can say which version each study used.
- After the study folder is renamed into place, import appends `{study: <name>, imported_at: <time>, asset_id: <value asset>, sha256: <the value asset's SHA-256>}` to the dataset manifest's `imports` list, under the series `manifest.lock` that ticket 1208 defines. A repeat import of the same study name with `--replace` replaces the existing entry rather than adding a second one. A failed import appends nothing. When the manifest cannot be locked or written, the study is already installed and the command exits nonzero with a message naming the manifest path and the study, so the user can re-run `study import --replace`.
- Output carries `data_as_of`, set to the import time, with `data_as_of_kind: "retrieved"`, following the definition ticket 1204 sets. GEO's own date is `upstream_last_updated` and travels as its own field, in `import.json`, in the `meta_study.txt` description, and in the output. It never fills `data_as_of`. Markdown ends with the GEO attribution line from ticket 1204.
...
Ticket 1218 changes how every study command failure renders, so that Markdown and JSON both name the missing file in place of the bare `cBioPortal DataHub is not available` line (`src/error.rs:482`, `:573`). That change is global and touches every existing study command, so it is its own ticket and this one depends on it. This ticket adds no rendering rule of its own. It relies on 1218 for the messages that name `meta_study.txt` and `data_mutations.txt` on an imported study.

### MCP arms

`is_allowed_mcp_command` (`src/mcp/shell.rs:472`) is an exhaustive match with no wildcard, so the new `StudyCommand::Import` variant is classified here or the build fails. It is rejected: it reads a workstation-local sample map and writes a study folder. `GENERIC_MCP_REJECTION_MESSAGE` (`src/mcp/shell.rs:326`) lists the allowed study commands by name and gains no entry for `study import`, because the command is rejected whole.
...
15. `study survival`, `study co-occurrence`, and `study compare --gene` on the imported study fail with a message that names the missing file, in Markdown and in JSON. A test pins both forms. The rendering itself is ticket 1218's; this item proves it holds for an imported study.
16. `--cancer-type AML` makes `study filter --cancer-type AML` return every imported sample.
17. `import.json` and the `meta_study.txt` description carry `upstream_last_updated` from the manifest's `series` block, including for an import whose value asset is an NCBI count file. A series block with a null value writes `null` in `import.json` and `unknown` in the description. Neither file sets `data_as_of` from that value; `data_as_of` is the import time with kind `retrieved`.
18. The dataset manifest gains one `imports` entry with the study name, the import time, the asset ID, and the SHA-256. An entry with a `previous` list records the top-level SHA-256, not a superseded one. A second import under the same name with `--replace` leaves one entry. A failed import leaves the list unchanged.
19. `study query --type expression` on the imported study makes no network request. The test asserts an empty request log.
20. The array table reader parses a fixture with a blank cell as missing, keeps a negative value as published, strips the quotes from a GSM column header, and reports a provider error naming the row number for a row whose column count differs from the header.
...
- Supplementary files with unknown meaning. Only 1207 products with `values_present` import.
