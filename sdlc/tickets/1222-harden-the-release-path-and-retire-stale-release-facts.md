...
- Correct the two stale documents and remove the vacuous test lines, keeping the assertions that can fail.
...
3. The two documents name v0.9.0 and the single workflow; the vacuous assertions are gone and the surviving ones still fail on a wrong sentence.
4. The package-boundary test passes on a clean checkout and does not depend on untracked files.
5. The cache-expiry test passes repeatedly under the full nextest run.
