...
A binary built from a working tree reports a version that differs from
the last published one — commit-suffixed, or a `-dev` patch bump. Two
builds from different commits never report the same string.
