...
- The 0.9.0 breaking refusal of `--status active` and its replacement `active_not_recruiting` appear in zero docs pages. The trial guide lists NCI mappings only (`docs/user-guide/trial.md:120-126`); the status table omits `active_not_recruiting` and `enrolling_by_invitation` (`docs/reference/quick-reference.md:222-236`).
...
- `docs/reference/mcp-server.md:17` still says v0.8.25 (ticket 1222, acceptance item 3).
...
- `docs/reference/release-process.md:75-76` version facts have no mechanical check.
...
- The `llms.txt` curated tool list and the `llms-full.txt` one-line descriptions are unguarded prose.

Enforcement gap:

- The CLI surface ratchet (`tests/test_cli_surface_contract_ratchet.py`, `tools/check-quality-ratchet.py:1017-1318`) checks alias tokens, entity command names in `cli-reference.md`, helper lists, JSON next commands, and shell-safe examples. It never reads per-entity guides, JSON field names, value vocabularies, version strings, `README.md`, or `llms.txt`, which is why the items above pass a green ratchet.

## Design

- Fix the shipped-but-undocumented items in the owning pages: trial guide and quick reference (status values, zero-result hint, `upstream_total`); entity tables in `README.md` and `docs/index.md`; quick reference and CLI reference (`author`, `study top-mutated`, `mcp tools`); `docs/policies.md` for the provider-capture store.
...
- Add `exclude_docs` for `docs/conftest.py`, and publish the changelog or state where release notes live.
- Extend the ratchet with the cheapest guards that bite: the trial status vocabulary, the `author` entity in the entity tables, and the release-process version facts. Keep each guard scoped so it fails on a wrong sentence, not on reflow.
...
1. Every command, entity, value, and flag named in the findings is documented correctly or removed from the docs.
2. `docs/reference/quick-reference.md` and the trial guide agree with `biomcp search trial --help` and the `--status active` refusal.
3. The ratchet carries at least the status vocabulary, the `author` entity, and the release-process version facts.
4. `https://biomcp.org/conftest.py` is gone after the next deploy and `mkdocs build --strict` passes.
