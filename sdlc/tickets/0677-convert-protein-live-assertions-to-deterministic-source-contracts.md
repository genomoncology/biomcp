...
Carried over from March ticket 677 when BioMCP moved to the sdlc
factory. The body below is March's, unchanged; it was already written to
stand alone. Work products from any earlier attempt:

    /home/ian/workspace/planning/biomcp/artifacts/677-convert-protein-live-assertions-to-deterministic-source-contracts
## Why
Ticket 645 classifies `spec/entity/protein.md` as convert: identity, structure, and complex-membership rendering are BioMCP contracts that currently lack dated capture evidence.

Split from ticket 668 on 2026-08-04. This is the smaller half — two providers — and should be the cheapest remaining conversion in the set.

## What
Add consumed Tier 2 plans for the UniProt and ComplexPortal paths. Record receipt-backed real captures for identity, structure, and complex membership, including empty and not-attempted outcomes. Prove the production transformations and local CLI rendering over those bytes, then convert the live blocks in `protein.md` while preserving the existing local ComplexPortal proof as routine coverage.

## Intermediate State
Protein identity, structure, and complex behavior is deterministic, and the existing local ComplexPortal assertions remain routine coverage in place.

## Scope

What is IN scope:
- `src/entities/protein.rs`, the UniProt and ComplexPortal source tests and captures, and the named blocks in `spec/entity/protein.md`.
- The `spec/entity/protein.md` entry in `SPEC_LIVE_PATHS`, in both `scripts/run-specs.sh` and `Makefile`, only after its replacements are green.

What is OUT of scope:
- `gene.md` and the MyGene/GO/HPA/NIH/GTR paths. That is a sibling ticket.
- New protein features, source additions, or presentation redesign.
...
- [ ] The existing local ComplexPortal assertions still run as routine proof and are not weakened or relocated.
...
## Notes
- See `architecture/technical/live-spec-conversion-target.md` for the `protein.md` row and its per-path prerequisites.
