...
`biomcp article citation-evidence <citing-id> <cited-id>` returns source text that supports the citation relationship when BioMCP can recover it. BioMCP uses the provider’s citation context when that context is useful. When the provider supplies no useful context and structured open full text is available, BioMCP resolves the cited reference and returns a bounded passage around a matching in-text citation marker.
...
- The two verified open-access examples recover passages linked to the requested cited references.
