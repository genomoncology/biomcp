...
- The first `workflow_dispatch` (run 35645761982, `container_only: true`, `tag: v0.9.0`) ran only `container-publish`, built and pushed the two-platform index, and left `latest` unmoved, but the amd64 smoke failed with `biomcp: /lib/x86_64-linux-gnu/libc.so.6: version 'GLIBC_2.39' not found (required by biomcp)`. The v0.9.0 Linux binaries need GLIBC_2.39 and the pinned runtime `debian:bookworm-slim` provides 2.36, so that push left an unusable `:0.9.0` tag for the re-dispatch to overwrite.
- The second `workflow_dispatch` (run 35649850201, `container_only: true`, `tag: v0.9.0`) failed with the same GLIBC_2.39 error on the amd64 smoke: the job checked out the release tag, so it built with the tag's `Dockerfile`, which still pins `debian:bookworm-slim@sha256:abd67ffc…`. Main's trixie `Dockerfile` never entered the build context, and the packaging fix cannot reach a tag build through the checkout.
...
- Add a `container-publish` job to `release.yml`:
  ...
  - Resolve the tag once as `github.event.release.tag_name || inputs.tag`. On `workflow_dispatch`, `GITHUB_REF_NAME` is the branch and `github.sha` is main, so the checkout takes the packaging from the workflow ref and `SOURCE_SHA` is the tag's commit read with `gh api "repos/${GITHUB_REPOSITORY}/commits/${TAG}" --jq .sha`; the image tags come from the resolved tag.
  - Check out the packaging ref with no `ref:` pin, so the `Dockerfile` and `.dockerignore` come from the workflow ref, then `gh release download` the two Linux tarballs, extract each `biomcp` into `dist/container/{amd64,arm64}/`, and write the sidecar from inside that directory, `(cd dist/container/$arch && sha256sum biomcp > biomcp.sha256)`. `Dockerfile:18` runs `sha256sum -c /tmp/biomcp.sha256` from `/usr/local/bin`, so a path-qualified line fails the build. Download all four Linux assets and run both published sidecar checks from `dist/` before extraction, so a corrupted or mismatched asset stops the job before it reaches the image. A `release: published` event runs from the tag, so normal releases are unchanged; a `workflow_dispatch --ref main` pairs main's packaging with the tag's content, which the v0.9.0 backfill needs because the tag's own `Dockerfile` still pinned the bookworm runtime.
  ...
  - Smoke both platforms by pulling and running `--version` from the registry, arm64 under QEMU, checking the running UID is not 0 and `org.opencontainers.image.revision` equals the tag's commit. Move `latest` with `docker buildx imagetools create` only after both smokes pass. Read the repository's latest release tag with `gh release view` and promote only when it equals the resolved tag, so a backfill for an older release keeps its versioned tag and leaves the shared `latest` pointer alone.
