...
Status: open. Reported 2026-09-24 during BioData ticket 0171 verification.

## Observation

A BioMCP spec artifact build ran on Yellow while BioData's reviewed container runner held the shared gate lock. The observer found BioMCP Cargo PID 1425695 and rustc PID 1425720 in the BioMCP checkout. Cargo stdout targeted `biomcp-gates-1240.log`. The Cargo command was `cargo build --locked --profile spec --no-default-features --bin biomcp --example rmcp_streamable_http_contract`. BioData's concurrent runner PID was 1420565 with Docker child 1420591.

The BioData run used pushed revision `ed42252b461287c67bc17e052e2731490755e78b`. Its partial formatting and fixture-policy failures are preliminary evidence. The run was stopped, its temporary checkout was cleaned up, and its results were not accepted as the required gate. The BioMCP processes were left untouched.
