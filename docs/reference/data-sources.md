...
| Pathway | Reactome + KEGG + g:Profiler | `https://reactome.org/ContentService`, `https://rest.kegg.jp`, `https://biit.cs.ut.ee/gprofiler/api` | No | Pathway search and detail use Reactome + KEGG; `events` and pathway `enrichment` are Reactome-only; top-level `biomcp enrich` uses **g:Profiler** |
