...
| Variant | MyVariant.info | `https://myvariant.info/v1` | No | rsID/HGVS lookup, ClinVar, population annotations, and prediction scores including separate `BayesDel add-AF` and `BayesDel no-AF` source values; BioMCP does not apply clinical thresholds or classify pathogenicity from them |
