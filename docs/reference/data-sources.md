...
| Variant population section | dbSNP RefSNP, gnomAD GraphQL API | `https://api.ncbi.nlm.nih.gov/variation/v0/beta`, `https://gnomad.broadinstitute.org/api` | No | rsID and gene/protein input can resolve one compatible GRCh38 chromosome placement through dbSNP before the direct `gnomad_r4` query; BioMCP does not liftover or infer an assembly from MyVariant and keeps exome/genome counts, ancestry frequencies, grpmax FAF95, and filters separate |
