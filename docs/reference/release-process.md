...
4. `pypi-build` builds five release-profile wheels: Linux x86_64, Linux ARM64, macOS arm64, macOS x86_64, and Windows x86_64. The Linux wheels build inside `quay.io/pypa/manylinux_2_28` containers — the glibc 2.28 floor is true by construction, a symbol scan enforces it against the shipped artifact, and the floor covers RHEL 8, Debian 10, and Ubuntu 20.04 onward. Older Linux users install from the GitHub tarball or `cargo install`; there is no sdist on purpose (a pip sdist install would require a user-side Rust toolchain). The Linux ARM64 leg assumes GitHub's free `ubuntu-24.04-arm` runners, available to public repositories.
5. `wheel-smoke` installs every wheel on its native runner and, for Linux, also installs and runs the wheel inside the matching manylinux_2_28 container to prove the floor at runtime. The deep trial and drug commands must exit 0; the absent-FAERS case must return its exact code and text; exit 101, a signal exit, stack-overflow text, malformed JSON, and missing skill assets fail the job.
