...
the `gnomad_r4` dataset and require a trustworthy GRCh38 coordinate. For an
rsID or gene/protein result whose MyVariant identity is not trustworthy GRCh38,
BioMCP first resolves a compatible chromosome placement through dbSNP RefSNP.
This replaces population fields previously copied from MyVariant.info payloads.
...
| `get variant <id> population` | Separate exome/genome frequency, ancestry counts, grpmax FAF95, and quality flags | Direct `gnomad_r4` GraphQL query; rsID and gene/protein inputs can resolve a dbSNP-backed GRCh38 coordinate first |
...
Returns direct gnomAD v4 exome and genome population results; when dbSNP resolves the rsID, JSON identifies the authoritative GRCh38 coordinate and section provenance names both dbSNP and gnomAD v4 while keeping raw numeric frequencies, counts, FAF95, ancestry rows, and source flag names.

```bash
biomcp get variant "chr7:g.140453136A>T" population
```

Returns the same population section after resolving a trustworthy GRCh38 identity; a GRCh37-only or ambiguous dbSNP result does not query gnomAD, BioMCP does not infer an assembly from MyVariant or liftover, and dbSNP transport or decoding failure is unavailable.
